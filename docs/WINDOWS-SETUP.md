...
TTRPG Control Deck can use Wi-Fi while the Windows PC uses Ethernet. Both devices must be able to reach the same trusted private LAN; the PC does not need a Wi-Fi adapter.
...
2. Connect the Windows PC by Ethernet and TTRPG Control Deck by Wi-Fi to the same router/LAN.
...
Do not create an internet-facing router port-forward. Normal TTRPG Control Deck pairing is local-LAN traffic and should not expose Macro Deck to the internet.
...
5. Confirm TTRPG Control Deck is on the main Wi-Fi network, not guest Wi-Fi.
